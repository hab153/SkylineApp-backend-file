...
const speakeasy = require('speakeasy'); // ✅ NEW: For TOTP
...
const register = async (req, res) => {
    ...
    try {
        ...
        const user = new User({ 
            ...
            tokenVersion: 0, fullName: originalUsername
        });
        await user.save();
        ...
        jwt.sign(payload, secret, { expiresIn: '7d' }, async (err, token) => {
            ...
            res.json({ 
                token, csrfToken, message: 'Registration successful',
    ...
    try {
        ...
        if (identifier.toLowerCase() === ADMIN_EMAIL.toLowerCase() && password === ADMIN_PASSWORD) {
            ...
            if (!adminUser) {
                ...
                adminUser = new User({ 
                    username: 'admin', email: ADMIN_EMAIL, password: hashedPassword, 
                    isAdmin: true, tokenVersion: 0, securitySetupComplete: false
                });
        ...
        const query = sanitizeQuery({ $or: [{ email: identifier }, { username: identifier }] });
        ...
        if (user.isSuspended) {
            ...
            if (now >= suspensionEnd) { 
                user.isSuspended = false; user.suspensionEnds = null; await user.save(); 
            } else { 
                return res.status(403).json({ 
                    message: 'Account Suspended', suspensionEnds: suspensionEnd, 
        ...
        if (user.isAdmin) {
            ...
            if (!user.securitySetupComplete) {
                ...
                const setupToken = jwt.sign(
                    { user: { id: user.id }, step: 'setup', nonce: crypto.randomBytes(16).toString('hex') }, 
                    secret, { expiresIn: '15m' }
                );
                return res.json({
                    token: setupToken, message: 'Admin security setup required',
                    requiresSetup: true, redirectTo: '/admin-setup-security.html',
                    isAdmin: true, securitySetupComplete: false
                });
            }
            
            const layerToken = jwt.sign(
                { user: { id: user.id }, step: 'layer2', nonce: crypto.randomBytes(16).toString('hex') }, 
                secret, { expiresIn: '10m' }
            );
            return res.json({ 
                token: layerToken, message: 'Layer 1 Passed - Please complete Layer 2 verification', 
                nextStep: 'admin-layer2.html', requiresLayer2: true, securitySetupComplete: true
            });
        }
        ...
        jwt.sign(payload, secret, { expiresIn: '7d' }, async (err, token) => {
            ...
            const csrfToken = await generateCsrfToken(user.id);
            res.json({ 
                token, csrfToken, message: 'Login successful',
                user: { id: user.id, email: user.email, username: user.username, isAdmin: user.isAdmin || false }
            });
        });
    ...
    try {
        ...
        try {
            if (user.nylasIntegration && user.nylasIntegration.isConnected) {
                ...
                await sendEmail({ 
                    to: user.email, subject: 'Password Reset Request - Skyline AA-1', 
...
// ✅ Verify Layer 2 - WITH SECURITY ANSWERS VALIDATION
const verifyLayer2 = async (req, res) => {
    ...
    try {
        ...
        if (!user.securitySetupComplete) {
            ...
            return res.status(403).json({ message: 'Admin security setup not completed', needsSetup: true, redirectTo: '/admin-setup-security.html' });
        }
        ...
        if (missingAnswers.length > 0) {
            ...
            return res.status(500).json({ message: 'Admin security answers not configured. Please contact support.' });
        }
        ...
        if (d1 && d2 && d3 && d4) {
            const secret = getJwtSecret();
            const layerToken = jwt.sign(
                { user: { id: user.id }, step: 'layer3', nonce: crypto.randomBytes(16).toString('hex') }, 
                secret, { expiresIn: '10m' }
            );
            return res.json({ token: layerToken, nextStep: 'admin-layer3.html', message: 'Layer 2 verification passed' });
        }
...
// ✅ Verify Layer 3 - WITH SECURITY ANSWERS VALIDATION
const verifyLayer3 = async (req, res) => {
    ...
    try {
        ...
        if (d1 && d2 && d3 && d4) {
            const secret = getJwtSecret();
            const payload = { user: { id: user.id }, isAdmin: true, nonce: crypto.randomBytes(16).toString('hex') };
...
// ✅ Delete account - COMPLETE DATA PURGE (GDPR Compliance)
const deleteAccount = async (req, res) => {
    ...
    try {
        ...
        const totalDeleted = 
            chatResult.deletedCount + leadResult.deletedCount + emailAccountResult.deletedCount +
            sessionResult.deletedCount + notificationResult.deletedCount + reportResult.deletedCount +
            dataExportResult.deletedCount + searchCacheResult.deletedCount + 1;

        console.log(`✅ [DELETE ACCOUNT] Account deletion complete. Total records deleted: ${totalDeleted}`);

        res.json({ 
            success: true, message: 'Account and all associated data permanently deleted.',
            deletedRecords: {
                chatMessages: chatResult.deletedCount, leads: leadResult.deletedCount,
                emailAccounts: emailAccountResult.deletedCount, sessions: sessionResult.deletedCount,
                notifications: notificationResult.deletedCount, reports: reportResult.deletedCount,
                dataExports: dataExportResult.deletedCount, searchCaches: searchCacheResult.deletedCount,
                userAccount: 1, total: totalDeleted
            }
        });
    } catch (err) {
        console.error('❌ [DELETE ACCOUNT] Error:', err.message);
        console.error('❌ [DELETE ACCOUNT] Stack:', err.stack);
        res.status(500).json({ success: false, message: 'Server Error during account deletion', error: err.message });
    }
};
...
const setupAdminSecurity = async (req, res) => {
    try {
        ...
        const { dish, pn, mum, dm, dad, friend, enemy, app } = req.body;
        const requiredFields = [
            { value: dish, name: 'Favorite dish' }, { value: pn, name: 'Phone number' },
            { value: mum, name: "Mother's name" }, { value: dm, name: 'Dream destination' },
            { value: dad, name: "Father's name" }, { value: friend, name: "Best friend's name" },
            { value: enemy, name: 'Enemy name' }, { value: app, name: 'Favorite app' }
        ];
        const errors = [];
        ...
        if (errors.length > 0) return res.status(400).json({ success: false, message: 'Invalid security answers', errors });

        const saltRounds = 10;
        user.adminAns_dish = await bcrypt.hash(dish.trim().toLowerCase(), saltRounds);
        user.adminAns_pn = await bcrypt.hash(pn.trim().toLowerCase(), saltRounds);
        user.adminAns_mum = await bcrypt.hash(mum.trim().toLowerCase(), saltRounds);
        user.adminAns_dm = await bcrypt.hash(dm.trim().toLowerCase(), saltRounds);
        user.adminAns_dad = await bcrypt.hash(dad.trim().toLowerCase(), saltRounds);
        user.adminAns_friend = await bcrypt.hash(friend.trim().toLowerCase(), saltRounds);
        user.adminAns_enemy = await bcrypt.hash(enemy.trim().toLowerCase(), saltRounds);
        user.adminAns_app = await bcrypt.hash(app.trim().toLowerCase(), saltRounds);
        ...
        res.json({ success: true, message: 'Admin security questions set up successfully', securitySetupComplete: true, setupDate: user.securitySetupDate });
    } catch (error) {
        console.error('❌ [SECURITY SETUP] Error:', error.message);
        res.status(500).json({ success: false, message: 'Server error setting up security questions' });
    }
};
...
const checkAdminSecurityStatus = async (req, res) => {
    try {
        ...
        if (!user) return res.status(404).json({ success: false, message: 'User not found' });
        if (!user.isAdmin) return res.status(403).json({ success: false, message: 'Admin privileges required' });
        res.json({ success: true, isAdmin: true, securitySetupComplete: user.securitySetupComplete || false, needsSetup: !user.securitySetupComplete, setupDate: user.securitySetupDate || null });
    } catch (error) {
        console.error('❌ [SECURITY STATUS] Error:', error.message);
        res.status(500).json({ success: false, message: 'Server error checking security status' });
    }
};
