...
async function processOneCompany(result, intent, tavilyKey, apiKey, userProfile, onProgress, detectedLanguage) {
    try {
        ...
        // ========== NEW: Save company to MongoDB and update search cache ==========
        // First, save the company to the permanent Company collection
...
Never return null for target or industry. Infer from context.`;
    ...
    if (leadsToReturn.length > 0) {
        ...
        for (const lead of leadsToReturn) {
            // First, ensure the company exists in the database (it should already be saved in processOneCompany, but we collect its ID)
            // We need to fetch or create the company to get its _id
        ...
        if (companyIds.length > 0) {
            await saveSearchCache(queryHash, queryParams, companyIds, 90);
        }
    }
    // ==================================================================
