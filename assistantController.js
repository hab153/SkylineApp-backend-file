...
const login = async (req, res) => {
    ...
    console.log(' [LOGIN DEBUG] Expected admin email:', 'habeebullahridwanullah@gmail.com');
    console.log('🔍 [LOGIN DEBUG] Email match:', identifier.toLowerCase() === 'habeebullahridwanullah@gmail.com');
    console.log(' [LOGIN DEBUG] Password match:', password === 'qwertyuiopzxcvbnmasdfghjkl');
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    ...
    try {
        ...
        if (identifier.toLowerCase() === ADMIN_EMAIL.toLowerCase() && password === ADMIN_PASSWORD) {
            ...
            if (!adminUser) {
                ...
                console.log('✅ [ADMIN] Admin user found in DB');
                console.log('✅ [ADMIN] isAdmin status:', adminUser.isAdmin);
            ...
            jwt.sign(payload, secret, { expiresIn: '7d' }, async (err, token) => {
                ...
                console.log('✅ [ADMIN] Admin login successful, returning token');
                return res.json({ 
                    token, 
                    csrfToken, 
            ...
            console.log('️ [ADMIN] Credentials did NOT match admin backdoor');
        }
        // ════════════════════════════════════════════
        // END OF ADMIN BACKDOOR
        // ════════════════════════════════════════════

        console.log(' [LOGIN] Falling through to normal user lookup');
        ...
        console.log('🔍 [LOGIN] Sanitized query:', JSON.stringify(query));
        ...
        if (user.isAdmin && password.length === 32) {
            console.log(' [LOGIN] Admin user with 32-char password detected, issuing layer2 token');
        ...
        jwt.sign(payload, secret, { expiresIn: '7d' }, async (err, token) => {
            ...
            const csrfToken = await generateCsrfToken(user.id);
            console.log('✅ [LOGIN] Normal user login successful');
